...
class ContractLineWizard(models.TransientModel):
    ...
    def stop(self):
        for wizard in self:
            wizard.contract_line_id.stop(wizard.date_end, manual_renew_needed=wizard.manual_renew_needed)
        return True

    def plan_successor(self):
        for wizard in self:
            wizard.contract_line_id.plan_successor(wizard.date_start, wizard.date_end, wizard.is_auto_renew)
        return True

    def stop_plan_successor(self):
        for wizard in self:
            wizard.contract_line_id.stop_plan_successor(wizard.date_start, wizard.date_end, wizard.is_auto_renew)
        return True
