...
class TestContractBase(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.pricelist = cls.env["product.pricelist"].create({"name": "pricelist for contract test"})
        ...
        cls.product_1.taxes_id += cls.env["account.tax"].search([("type_tax_use", "=", "sale")], limit=1)
        ...
        cls.terminate_reason = cls.env["contract.terminate.reason"].create({"name": "terminate_reason"})
...
class TestContract(TestContractBase):
    ...
    def test_add_modifications(self):
        ...
        self.contract.message_subscribe(partner_ids=partner2.ids, subtype_ids=self.env.ref("mail.mt_comment").ids)
        subtype = self.env.ref("contract.mail_message_subtype_contract_modification")
        partner_ids = self.contract.message_follower_ids.filtered(lambda x: subtype in x.subtype_ids).mapped(
            "partner_id"
        )
    ...
    def test_contract_invoice_followers(self):
        ...
        subtype_ids.append(self.env.ref("contract.mail_message_subtype_invoice_created").id)
        self.contract.message_subscribe(partner_ids=self.contract.partner_id.ids, subtype_ids=subtype_ids)
    ...
    def test_contract_invoice_salesperson(self):
        self.acct_line.recurring_next_date = "2018-02-23"
        self.acct_line.recurring_rule_type = "daily"
        new_salesperson = self.env["res.users"].create({"name": "Some Salesperson", "login": "salesperson_test"})
    ...
    def test_check_recurring_next_date_start_date(self):
        with self.assertRaises(ValidationError):
            self.acct_line.write({"date_start": "2018-01-01", "recurring_next_date": "2017-01-01"})
    ...
    def test_contract_onchange_product_id_uom(self):
        ...
        line = self._add_template_line({"uom_id": self.env.ref("uom.product_uom_litre").id})
    ...
    def test_contract_onchange_product_id_name(self):
        ...
        line._onchange_product_id()
        self.assertEqual(line.name, line.product_id.get_product_multiline_description_sale())
    ...
    def test_act_show_contract(self):
        show_contract = self.partner.with_context(contract_type="sale").act_show_contract()
        self.assertEqual(
            show_contract,
            {
                **show_contract,
    ...
    def test_get_default_recurring_invoicing_offset(self):
        ...
        self.assertEqual(clm._get_default_recurring_invoicing_offset("pre-paid", "monthly"), 0)
        self.assertEqual(clm._get_default_recurring_invoicing_offset("post-paid", "monthly"), 1)
        self.assertEqual(clm._get_default_recurring_invoicing_offset("pre-paid", "monthlylastday"), 0)
        self.assertEqual(
    ...
    def test_get_next_invoice_date(self):
        ...
        def error_message(
            ...
            return "Error in %s-%d every %d %s case, " "start with %s (max_date_end=%s)" % (
                recurring_invoicing_type,
                recurring_invoicing_offset,
                recurring_interval,
                recurring_rule_type,
    ...
    def test_stop_past_contract_line(self):
        ...
        self.acct_line.write({"date_end": self.today + relativedelta(months=5), "is_auto_renew": True})
    ...
    def test_stop_plan_successor_contract_line_1(self):
        ...
        * contract line end's before the suspension period:
        ...
        new_line = self.env["contract.line"].search([("predecessor_contract_line_id", "=", self.acct_line.id)])
    ...
    def test_stop_plan_successor_contract_line_2(self):
        ...
        * contract line start before the suspension period and end in it
        ...
        self.acct_line.stop_plan_successor(suspension_start, suspension_end, True)
        self.assertEqual(self.acct_line.date_end, suspension_start - relativedelta(days=1))
        new_line = self.env["contract.line"].search([("predecessor_contract_line_id", "=", self.acct_line.id)])
        self.assertTrue(new_line)
        new_date_end = suspension_end + (end_date - suspension_start) + relativedelta(days=1)
    ...
    def test_stop_plan_successor_contract_line_3(self):
        ...
        * contract line start before the suspension period and end after it
        ...
        new_line = self.env["contract.line"].search([("predecessor_contract_line_id", "=", self.acct_line.id)])
        self.assertTrue(new_line)
        new_date_end = end_date + (suspension_end - suspension_start) + relativedelta(days=1)
        self.assertEqual(new_line.date_start, suspension_end + relativedelta(days=1))
        self.assertEqual(new_line.date_end, new_date_end)
        self.assertTrue(self.acct_line.manual_renew_needed)
    ...
    def test_check_has_not_date_end_has_successor(self):
        self.acct_line.write({"date_end": False, "is_auto_renew": False})
        with self.assertRaises(ValidationError):
            self.acct_line.plan_successor(to_date("2016-03-01"), to_date("2016-09-01"), False)
    ...
    def test_check_has_successor_is_auto_renew(self):
        with self.assertRaises(ValidationError):
            self.acct_line.plan_successor(to_date("2016-03-01"), to_date("2018-09-01"), False)
    ...
    def test_check_last_date_invoiced_1(self):
        ...
        """
        with self.assertRaises(ValidationError):
            self.acct_line.write({"last_date_invoiced": self.acct_line.date_start - relativedelta(days=1)})
    ...
    def test_check_last_date_invoiced_2(self):
        ...
        with self.assertRaises(ValidationError):
            self.acct_line.write({"last_date_invoiced": self.acct_line.date_end + relativedelta(days=1)})
    ...
    def test_action_uncancel(self):
        ...
        self.assertEqual(action["context"]["default_contract_line_id"], self.acct_line.id)
    ...
    def test_purchase_get_view(self):
        ...
        view = self.acct_line.with_context(default_contract_type="purchase").get_view(view_type="tree")
        self.assertEqual(view["id"], purchase_tree_view.id)
        view = self.acct_line.with_context(default_contract_type="purchase").get_view(view_type="form")
    ...
    def test_multicompany_partner_edited(self):
        ...
        parent_partner = self.env["res.partner"].create({"name": "parent partner", "is_company": True})
        # Assume contract 2 is for company 2
        self.contract2.company_id = company2
        # Update the partner attached to both contracts
        self.partner.with_user(unprivileged_user).with_company(company2).with_context(company_id=company2.id).write(
    ...
    def test_sale_get_view(self):
        ...
        view = self.acct_line.with_context(default_contract_type="sale").get_view(view_type="form")
    ...
    def test_currency(self):
        ...
        self.assertEqual(self.contract2.currency_id, self.contract2.company_id.currency_id)
        ...
        pricelist = self.env["product.pricelist"].create({"name": "Test pricelist", "currency_id": currency_eur.id})
