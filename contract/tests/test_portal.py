...
class TestContractPortal(HttpCase):
    def test_tour(self):
        ...
        contract = self.env["contract.contract"].create({"name": "Test Contract", "partner_id": partner.id})
        ...
        url_contract = f"/my/contracts/{contract.id}?access_token={contract.access_token}"
