...
class ContractMulticompanyCase(TestContractBase):
    ...
    def setUpClass(cls):
        ...
        chart_template = cls.env["account.chart.template"]._guess_chart_template(cls.company_2.country_id)
        cls.env["account.chart.template"].try_loading(chart_template, company=cls.company_2, install_demo=False)
        ...
        cls.acct_line_mc = cls.env["contract.line"].with_company(cls.company_2).create(cls.line_vals)
    ...
    def test_cron_recurring_create_invoice_multi_company(self):
        ...
        for _i in range(10):
            ...
            contracts_company_2 |= self.contract_mc.with_company(company=self.company_2).create(vals)
        ...
        self.assertEqual(len(contracts.mapped("contract_line_ids")), len(invoice_lines_company_1))
        self.assertEqual(
