...
class ContractRecurrencyBasicMixin(models.AbstractModel):
    ...
    recurring_invoicing_type = fields.Selection(
        ...
        help=(
            "Specify if the invoice must be generated at the beginning " "(pre-paid) or end (post-paid) of the period."
        ),
    )
    ...
    def _compute_recurring_invoicing_offset(self):
        for rec in self:
            ...
            rec.recurring_invoicing_offset = method(rec.recurring_invoicing_type, rec.recurring_rule_type)

    @api.model
    def _get_default_recurring_invoicing_offset(self, recurring_invoicing_type, recurring_rule_type):
        if recurring_invoicing_type == "pre-paid" or recurring_rule_type == "monthlylastday":
            return 0
        else:
            return 1
...
class ContractRecurrencyMixin(models.AbstractModel):
    ...
    recurring_next_date = fields.Date(compute="_compute_recurring_next_date", store=True, readonly=False, copy=True)
    ...
    def get_next_invoice_date(
        ...
        if not next_period_date_end:
            return False
        if recurring_invoicing_type == "pre-paid":
            recurring_next_date = next_period_date_start + relativedelta(days=recurring_invoicing_offset)
        else:  # post-paid
