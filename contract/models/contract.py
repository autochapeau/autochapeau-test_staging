...
class ContractContract(models.Model):
    ...
    contract_template_id = fields.Many2one(string="Contract Template", comodel_name="contract.template")
    ...
    payment_term_id = fields.Many2one(comodel_name="account.payment.term", string="Payment Terms", index=True)
    ...
    partner_id = fields.Many2one(comodel_name="res.partner", inverse="_inverse_partner_id", required=True)
    ...
    # Sales fields
    sales_representative_id = fields.Many2one("res.partner")
    sales_representative_ids = fields.Many2many("res.partner", string="Sales Representatives")
    ...
    def write(self, vals):
        if "modification_ids" in vals:
            res = super(ContractContract, self.with_context(bypass_modification_send=True)).write(vals)
    ...
    def _set_start_contract_modification(self):
        ...
        for record in self:
            ...
            record.message_subscribe(partner_ids=[record.partner_id.id], subtype_ids=[subtype_id.id])
            record.with_context(skip_modification_mail=True).write(
                {"modification_ids": [(0, 0, {"date": date_start, "description": _("Contract start")})]}
            )

    @api.model
    def _modification_mail_send(self):
        for record in self:
            modification_ids_not_sent = record.modification_ids.filtered(lambda x: not x.sent)
            if modification_ids_not_sent:
                if not self.env.context.get("skip_modification_mail"):
                    subtype_id = self.env["ir.model.data"]._xmlid_to_res_id(
                        "contract.mail_message_subtype_contract_modification"
                    )
                    template_id = self.env.ref("contract.mail_template_contract_modification")
    ...
    def _get_computed_currency(self):
        ...
        if any(self.contract_line_ids.mapped("automatic_price")):
            ...
            currency = (
                ...
                or self.partner_id.with_company(self.company_id).property_product_pricelist.currency_id
            )
    ...
    def action_show_invoices(self):
        ...
        if ctx.get("default_contract_type"):
            ctx["default_move_type"] = "out_invoice" if ctx.get("default_contract_type") == "sale" else "in_invoice"
        action = {
    ...
    def _compute_recurring_next_date(self):
        for contract in self:
            ...
            if contract._origin and contract._origin.date_start != contract.date_start or not recurring_next_date:
    ...
    def _compute_create_invoice_visibility(self):
        for contract in self:
            contract.create_invoice_visibility = any(contract.contract_line_ids.mapped("create_invoice_visibility"))
    ...
    def _onchange_partner_id(self):
        partner = self.partner_id if not self.company_id else self.partner_id.with_company(self.company_id)
        self.pricelist_id = partner.property_product_pricelist.id
        self.fiscal_position_id = partner.env["account.fiscal.position"]._get_fiscal_position(partner)
    ...
    def _prepare_invoice(self, date_invoice, journal=None):
        ...
        if not journal:
            raise ValidationError(
                _("Please define a %(contract_type)s journal " "for the company '%(company)s'.")
                % {
    ...
    def _invoice_followers(self, invoices):
        invoice_create_subtype = self.env.ref("contract.mail_message_subtype_invoice_created")
        for item in self:
            partner_ids = item.message_follower_ids.filtered(lambda x: invoice_create_subtype in x.subtype_ids).mapped(
                "partner_id"
            )
            if partner_ids:
                (invoices & item._get_related_invoices()).message_subscribe(partner_ids=partner_ids.ids)
    ...
    def _cron_recurring_create(self, date_ref=False, create_type="invoice"):
        ...
        from contracts.
        """
        _recurring_create_func = self._get_recurring_create_func(create_type=create_type)
        if not date_ref:
            date_ref = fields.Date.context_today(self)
        ...
        for company in companies:
            contracts_to_invoice = contracts.filtered(
                ...
                and (not contract.date_end or contract.recurring_next_date <= contract.date_end)
