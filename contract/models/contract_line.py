...
class ContractLine(models.Model):
    ...
    is_plan_successor_allowed = fields.Boolean(string="Plan successor allowed?", compute="_compute_allowed")
    is_stop_plan_successor_allowed = fields.Boolean(string="Stop/Plan successor allowed?", compute="_compute_allowed")
    is_stop_allowed = fields.Boolean(string="Stop allowed?", compute="_compute_allowed")
    is_cancel_allowed = fields.Boolean(string="Cancel allowed?", compute="_compute_allowed")
    is_un_cancel_allowed = fields.Boolean(string="Un-Cancel allowed?", compute="_compute_allowed")
    ...
    def _compute_state(self):
        ...
        for rec in self:
            ...
            if rec.date_start and rec.date_start > today:
                ...
                rec.state = "upcoming"
                continue
            if rec.date_start and rec.date_start <= today and (not rec.date_end or rec.date_end >= today):
                # In period
                if (
            ...
            if rec.date_end and rec.date_end < today:
                # After
                if rec.manual_renew_needed and not rec.successor_contract_line_id or rec.is_auto_renew:
                    rec.state = "to-renew"
                else:
                    rec.state = "closed"
    ...
    def _search_state(self, operator, value):
        ...
        if operator == "not in":
            if set(value) == set(states):
                return [("id", "=", False)]
            return self._search_state("in", [state for state in states if state not in value])
        ...
        for rec in self:
            if rec.is_auto_renew:
                if rec.successor_contract_line_id:
                    raise ValidationError(_("A contract line with a successor " "can't be set to auto-renew"))
                if not rec.date_end:
                    raise ValidationError(_("An auto-renew line must have a end date"))
            else:
                if not rec.date_end and rec.successor_contract_line_id:
                    raise ValidationError(_("A contract line with a successor " "must have a end date"))
        ...
        for rec in self:
            if rec.date_end and rec.successor_contract_line_id:
                if rec.date_end >= rec.successor_contract_line_id.date_start:
                    raise ValidationError(_("Contract line and its successor overlapped"))

    @api.constrains("predecessor_contract_line_id", "date_start")
    def _check_overlap_predecessor(self):
        for rec in self:
            if rec.predecessor_contract_line_id and rec.predecessor_contract_line_id.date_end:
                if rec.date_start <= rec.predecessor_contract_line_id.date_end:
                    raise ValidationError(_("Contract line and its predecessor overlapped"))
    ...
    def _compute_first_recurring_next_date(
        ...
        return self.get_next_invoice_date(
            date_start,
            recurring_invoicing_type,
            self._get_default_recurring_invoicing_offset(recurring_invoicing_type, recurring_rule_type),
            recurring_rule_type,
            recurring_interval,
            max_date_end=False,
        )
    ...
    @api.model
    def _get_first_date_end(self, date_start, auto_renew_rule_type, auto_renew_interval):
        return date_start + self.get_relative_delta(auto_renew_rule_type, auto_renew_interval) - relativedelta(days=1)
    ...
    def _check_auto_renew_canceled_lines(self):
        for rec in self:
            if rec.is_canceled and rec.is_auto_renew:
                raise ValidationError(_("A canceled contract line can't be set to auto-renew"))

    @api.constrains("recurring_next_date", "date_start")
    def _check_recurring_next_date_start_date(self):
        for line in self:
            if line.display_type in ("line_section", "line_note") or not line.recurring_next_date:
                continue
            if line.date_start and line.recurring_next_date:
                if line.date_start > line.recurring_next_date:
                    raise ValidationError(
                        _("You can't have a date of next invoice anterior " "to the start of the contract line '%s'")
                        % line.name
                    )

    @api.constrains("date_start", "date_end", "last_date_invoiced", "recurring_next_date")
    def _check_last_date_invoiced(self):
        for rec in self.filtered("last_date_invoiced"):
            if rec.date_end and rec.date_end < rec.last_date_invoiced:
                raise ValidationError(
                    _("You can't have the end date before the date of last " "invoice for the contract line '%s'")
                    % rec.name
                )
            ...
            if rec.date_start and rec.date_start > rec.last_date_invoiced:
                raise ValidationError(
                    _("You can't have the start date after the date of last " "invoice for the contract line '%s'")
                    % rec.name
                )
            if rec.recurring_next_date and rec.recurring_next_date <= rec.last_date_invoiced:
                raise ValidationError(
                    _(
    ...
    def _check_recurring_next_date_recurring_invoices(self):
        for rec in self:
            if not rec.recurring_next_date and (
                not rec.date_end or not rec.last_date_invoiced or rec.last_date_invoiced < rec.date_end
            ):
                raise ValidationError(_("You must supply a date of next invoice for contract " "line '%s'") % rec.name)
        ...
        for line in self.filtered("date_end"):
            if line.date_start and line.date_end:
                if line.date_start > line.date_end:
                    raise ValidationError(
                        _("Contract line '%s' start date can't be later than" " end date") % line.name
                    )
    ...
    def _compute_create_invoice_visibility(self):
        ...
        today = fields.Date.context_today(self)
        for rec in self:
            if (not rec.display_type or rec.is_recurring_note) and rec.date_start and today >= rec.date_start:
    ...
    def _get_period_to_invoice(self, last_date_invoiced, recurring_next_date, stop_at_date_end=True):
        # TODO this method can now be removed, since
        # TODO self.next_period_date_start/end have the same values
        self.ensure_one()
        if not recurring_next_date:
            return False, False, False
        first_date_invoiced = last_date_invoiced + relativedelta(days=1) if last_date_invoiced else self.date_start
    ...
    def _insert_markers(self, first_date_invoiced, last_date_invoiced):
        ...
        name = name.replace(
            "#INVOICEMONTHNAME#",
            self.with_context(lang=lang.code)._translate_marker_month_name(first_date_invoiced.strftime("%B")),
        )
        return name
    ...
    def _delay(self, delay_delta):
        ...
        for rec in self:
            if rec.last_date_invoiced:
                raise ValidationError(_("You can't delay a contract line " "invoiced at least one time."))
        ...
        for rec in self:
            if date_end < rec.date_start:
                ...
                if not rec.date_end or rec.date_end > date_end:
                    ...
                    rec.write(rec._prepare_value_for_stop(date_end, manual_renew_needed))
                    if post_message:
                        msg = _(
    ...
    def _prepare_value_for_plan_successor(self, date_start, date_end, is_auto_renew, recurring_next_date=False):
        self.ensure_one()
        if not recurring_next_date:
            recurring_next_date = self.get_next_invoice_date(
                date_start,
    ...
    def plan_successor(
        ...
        for rec in self:
            ...
            new_line = self.create(
                rec._prepare_value_for_plan_successor(date_start, date_end, is_auto_renew, recurring_next_date)
            )
    ...
    def stop_plan_successor(self, date_start, date_end, is_auto_renew):
        ...
        for rec in self:
            if rec.date_start >= date_start:
                ...
                if rec.date_end and rec.date_end < date_start:
                ...
                elif rec.date_end and rec.date_end > date_start and rec.date_end < date_end:
                    new_date_start = date_end + relativedelta(days=1)
                    new_date_end = date_end + (rec.date_end - date_start) + relativedelta(days=1)
                    rec.stop(
                    ...
                    new_date_start = date_end + relativedelta(days=1)
                    if rec.date_end:
                        new_date_end = rec.date_end + (date_end - date_start) + relativedelta(days=1)
                    else:
    ...
    def cancel(self):
        ...
        for contract in self.mapped("contract_id"):
            ...
            msg = _(
                ...
                "<br/>- ".join([f"<strong>{name}</strong>" for name in lines.mapped("name")]),
            )
            contract.message_post(body=msg)
        self.mapped("predecessor_contract_line_id").write({"successor_contract_line_id": False})
    ...
    def action_plan_successor(self):
        ...
        context.update(self.env.context)
        view_id = self.env.ref("contract.contract_line_wizard_plan_successor_form_view").id
        return {
            "type": "ir.actions.act_window",
    ...
    def action_stop_plan_successor(self):
        ...
        context.update(self.env.context)
        view_id = self.env.ref("contract.contract_line_wizard_stop_plan_successor_form_view").id
        return {
            "type": "ir.actions.act_window",
    ...
    def _get_renewal_new_date_end(self):
        ...
        date_end = self._get_first_date_end(date_start, self.auto_renew_rule_type, self.auto_renew_interval)
        return date_end
    ...
    def _renew_create_line(self, date_end):
        ...
        new_line = self.plan_successor(date_start, date_end, is_auto_renew, post_message=False)
        return new_line
    ...
    def _get_quantity_to_invoice(self, period_first_date, period_last_date, invoice_date):
        self.ensure_one()
