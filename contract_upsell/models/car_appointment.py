...
class CarAppointment(models.Model):
    ...
    def create(self, vals_list):
        ...
        for appointment in appointments:
            ...
            if not sale or sale.order_type != "contract":
                continue
            # Ensure extras created before/with the appointment are included.
            sale.child_sale_ids.filtered(
                lambda order: order.state != "cancel"
            )._sync_contract_upsell_to_appointment()
        return appointments
    ...
    def action_done(self):
        ...
        for appointment in self:
            ...
            # Appointment lines first (visibility), then WO operational lines.
            extras._sync_contract_upsell_to_visit()
        return result
